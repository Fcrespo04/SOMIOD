...
namespace MiddleWare.Helpers
{
    public static class BD_Access
    {
        ...
        // Method to send notifications to subscribers
        public static void SendNotifications(int containerId, int evtType, object resourceData)
        {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                ...
                // 1. Obter nomes da App e Container para construir o tópico (Channel Name)
                // O tópico deve ser: api/somiod/application/container
                var sqlInfo = "SELECT a.[resource-name] as app, c.[resource-name] as cont " +
                              "FROM container c JOIN application a ON c.parent=a.id WHERE c.id=@pid";

                using (var cmdInfo = new SqlCommand(sqlInfo, conn))
                {
                    cmdInfo.Parameters.AddWithValue("@pid", containerId);
                    using (var rInfo = cmdInfo.ExecuteReader())
                    {
                        if (rInfo.Read())
                        {
                            appName = (string)rInfo["app"];
                            contName = (string)rInfo["cont"];
                        }
                        else return; // Se não encontrar o contentor, aborta
                    }
                }

                // 2. Preparar a mensagem (XML)
                // O enunciado pede que o recurso e o tipo de evento façam parte da notificação.
                string eventName = (evtType == 1) ? "CREATE" : "DELETE";

                // Serialização manual simples para garantir formato XML
                // Se resourceData for um objeto complexo, o .ToString() pode não ser suficiente, 
                // mas funciona se for string (XML) ou se serializares o objeto antes de chamar este método.
                string xmlMessage = $"<notification>" +
                                        $"<event>{eventName}</event>" +
                                        $"<resource>{resourceData}</resource>" +
                                    $"</notification>";

                // 3. Buscar subscrições e enviar
                var sql = "SELECT endpoint FROM subscription WHERE parent=@pid AND (evt=@type OR evt=0)";
                var cmd = new SqlCommand(sql, conn);
                ...
                using (var r = cmd.ExecuteReader())
                {
                    while (r.Read())
                    {
                        ...
                        {
                            // Tópico conforme enunciado: path para o container
                            string topic = $"api/somiod/{appName}/{contName}";

                            // Usa o MqttHelper que já tens
                            // Nota: O helper atualmente usa IP 127.0.0.1 fixo internamente, 
                            // mas passamos o endpoint na mesma caso mudes a lógica lá depois.
                            MqttHelper.PublishToBroker(endpoint, topic, xmlMessage);

                            System.Diagnostics.Debug.WriteLine($"[NOTIFY MQTT] Tópico: {topic} | Msg: {xmlMessage}");
                        }
                        // B. Envio via HTTP
                                 ...
                                 endpoint.StartsWith("https://", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            System.Threading.Tasks.Task.Run(async () =>
                            {
                                try
                                {
                                    ...
                                    {
                                        var content = new System.Net.Http.StringContent(xmlMessage, System.Text.Encoding.UTF8, "application/xml");
                                        var response = await client.PostAsync(endpoint, content);
                                        System.Diagnostics.Debug.WriteLine($"[NOTIFY HTTP] {endpoint}: {response.StatusCode}");
                                    }
                                }
                                catch (Exception ex)
                                {
                                    System.Diagnostics.Debug.WriteLine($"[NOTIFY HTTP ERRO] {ex.Message}");
                                }
                            });
                        }
                    }
